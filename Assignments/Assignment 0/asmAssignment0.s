...
do_Str:                        		; do_Str function definition - functions are defined as labels
	...
	yourCode:			; use label to build a loop for treating the input string characters
		cmp byte [ecx], 32	; compare the current char to SPACE
		je equalsToSpaceOrTAB	; go to equals sector if the char is SPACE
		cmp byte [ecx], 9	; compare the current char to SPACE
		je equalsToSpaceOrTAB	; go to equals sector if the char is SPACE
		jne bottom		; go to equals sector if the char is not SPACE	

	equalsToSpaceOrTAB:
		inc dword [an]		; increament al by 1
		mov byte [ecx], 95	; replace the SPACE char to '_'
