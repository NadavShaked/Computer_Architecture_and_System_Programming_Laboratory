...
char *words(int count) {
  ...
  if(count==1) {
    words = "word";
    //The reason that the code doesn't works is because that any char of the string is constant
    //so we have to change the whole string or to allocate writeable memory by malloc()
